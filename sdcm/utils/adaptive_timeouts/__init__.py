    ...
    def _monitor_timeouts(self):
        while self.running:
            ...
            if self.hard_timeout and duration > self.hard_timeout:
                HardTimeoutEvent(
                    operation=self.operation, duration=duration, hard_timeout=self.hard_timeout
                ).publish_or_dump()
                self.running = False
                return
...
def adaptive_timeout(  # noqa: PLR0914
    ...
    **kwargs,
    ...
    monitor_ctx = (
        TimeoutMonitor(operation.name, soft_timeout, hard_timeout, start_time=start_time)
