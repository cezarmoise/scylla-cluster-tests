...
class Nemesis:  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def grow_incremental_instances(self):
        ...
        def upgrade_cluster(node_map):
            ...
            for i in range(1, len(sorted_nodes) - 2):
        ...
        def write_data(n):
            ...
            for stress_cmd in stress_cmds:
                ...
                pre_create_keyspace = pre_create_keyspace.replace("keyspace1", f"keyspace{n}")
                with self.cluster.cql_connection_patient(self.target_node, connect_timeout=600) as session:
        ...
        target_size = 3 * instance_size_map["i4i.8xlarge"]  # 3 instances of i4i.8xlarge = 48 units
        node_map = {node: "i4i.large" for node in self.cluster.nodes}
        ...
        while current_size < target_size:
            ...
            current_size = sum(instance_size_map[instance] for instance in node_map.values())
            self.log.info(
                f"Old cluster: (Size: {old_size}) {list(sorted(old_node_map.values(), key=lambda x: instance_size_map[x]))} ")
            self.log.info(
                f"New cluster: (Size: {current_size}) {list(sorted(node_map.values(), key=lambda x: instance_size_map[x]))} ")
