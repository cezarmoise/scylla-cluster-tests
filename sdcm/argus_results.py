...
def submit_results_to_argus(argus_client: ArgusClient, result_table: GenericResultTable):
    try:
        ...
        if exc.args[1] == "DataValidationError":
            FailedResultEvent(
                f"Argus validation failed for the result in {result_table.name}."
                f" Please check the 'Results' tab for more details."
            ).publish()
        else:
            raise
...
def send_result_to_argus(  # noqa: PLR0914
    ...
    for event in result["reactor_stalls_stats"]:  # each stall event has own table
        ...
        result_table.add_result(column="total", row=cycle, value=stall_stats["counter"], status=Status.UNSET)
        for interval, value in stall_stats["ms"].items():
            result_table.add_result(column=f"{interval}ms", row=cycle, value=value, status=Status.UNSET)
        submit_results_to_argus(argus_client, result_table)
...
def send_manager_benchmark_results_to_argus(
    argus_client: ArgusClient, result: dict, result_table: StaticGenericResultTable, row_name: str = None
) -> None:
    if not row_name:
        row_name = "#1"
...
class PeriodicDiskUsageToArgus:
    ...
    def results_table(self):
        ...
        columns = [ColumnMetadata(name="time", unit="", type=ResultType.TEXT),]
