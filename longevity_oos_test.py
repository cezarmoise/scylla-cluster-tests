...
class LongevityOutOfSpaceTest(LongevityTest):
    ...
    #     # start a thread to monitor disk usage
    #     self.stop_monitor_thread = Event()
    #     self.disk_usage_monitor_thread = Thread(target=self.monitor_disk_usage, daemon=True)
    #     self.disk_usage_monitor_thread.start()

    # def tearDown(self):
    #     super().tearDown()
    #     # kill the disk usage monitor thread
    #     self.stop_monitor_thread.set()
    #     self.disk_usage_monitor_thread.join(timeout=120)

    # def monitor_disk_usage(self):
    #     sleep(600)  # wait for nodes to have prometheus_db metrics
    ...
    #     :param interval: The time interval for the query, defaults to 60 seconds.
    ...
    #         TestFrameworkEvent(source=self.__class__.__name__, message=f"Repair should not finish. Status: {task_final_status}",
    ...
    #         lose multiple nodes in a rack simultaneously
    ...
    #     node1 = rack1[0]
    #     # lose all but one node in the rack simultaneously
    ...
    #     # Perform removenode operations for all terminated nodes
    #     for node, host_id in host_ids_to_remove.items():
    #         self.log.info(f"Removing node {node.name} with {host_id=} from the cluster")
